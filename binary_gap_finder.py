# binary gap finder
# find the length of the longest group of consequtive zeros in a binary number, with ones on both sides.
# example, 110003001 has a length of 3.  1001 has a length of 2.  10101 has a length of 1.  100 has a length of zero (no one on the one side).
...
def main():
   ...
   print("The binary representation is " + convert_binary_number_in_progress_to_string(binary_number_and_gap[1]) + ".")
...
def determine_binary_and_gap(current_target, binary_number_in_progress, largest_gap_yet, current_size_of_gap, is_gap_right_now, powers_of_two):
    ...
    if len(powers_of_two) > 0:
        return determine_binary_and_gap(current_target, binary_number_in_progress, largest_gap_yet, current_size_of_gap, is_gap_right_now, powers_of_two)
    else:
        return (largest_gap_yet, binary_number_in_progress)

def convert_binary_number_in_progress_to_string(binary_in_progress):
